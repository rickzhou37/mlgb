...
class MultigateMixtureOfExpertLayer(tf.keras.layers.Layer):
    ...
    @tf.function
    def call(self, inputs):
        x = inputs # [batch_size, unit]
        x_experts = tf.stack([x] * self.expert_num, axis=1) # (b, experts_num, unit)
...
class ParameterEmbeddingPersonalizedNetworkLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, task_fn_list, domain_num=1, tower_dnn_hidden_units=(64, 32),
        ...
        # 每层一个 gate， 每个 gate 的输出和层的输出一样
        # n1 和 n2 组合组成 gate NU (第一层是 relu，第二层是 sigmoid)
        self.gnu_n1_fn_list = [
            DeepNeuralNetworkLayer(
        ...
        # 每个 task 一个 tower （tower_dnn_length 层）
        self.tower_dnn_fn_list = [
            [
                DeepNeuralNetworkLayer(
                    dnn_hidden_units=[self.tower_dnn_hidden_units[i]],
    ...
    def build(self, input_shape):
        # rank 为什么等于 3 ？
        # [general_input_shape, domain_input_shape, user_item_author_input_shape]
        if len(input_shape) != 3:
            raise MLGBError

        # general input: (batch_size, general_fea_size, emb_dim)
        # domain input: (batch_size, domain_fea_size, emb_dim)
        # user_item_author_input: (batch_size, user_item_author_fea_size, emb_dim)
    ...
    def call(self, inputs):
        ...
        print("#PEPNetLayer 1, x_g shape: ", x_g.shape)
        print("#PEPNetLayer 2, x_d shape: ", x_d.shape)
        print("#PEPNetLayer 3, x_uia shape: ", x_uia.shape)

        # epnet 是否 concat domain 和 general
        # 如果 concat general，阻止梯度回传影响 general emb
        if self.gnu_if_concat_general_inputs:
            x_g_ep = self.stop_gradient_fn_list[0](x_g)  # EPNet
            x_d = tf.concat([x_d, x_g_ep], axis=1) # (?, domain_fea_num + general_fea_num, emb_dim)

        # 相同 shape 元素乘 ？
        x_d = self.gnu_0_fn(x_d) # （batch, domain_fea_num, emb_dim）
        print("#PEPNetLayer 4, gnu x_d shape: ", x_d.shape)
        x_g = tf.einsum('bie,bje->bije', x_g, x_d)
        print("#PEPNetLayer 5, epnet1 x_g shape: ", x_g.shape)
        x_g = self.flatten_axes_fn(x_g) # （batch, g_fea_num * domain_fea_num, emb_dim）
        print("#PEPNetLayer 6, epnet2 x_g shape: ", x_g.shape)
