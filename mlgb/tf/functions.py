...
class SparseEmbeddingLayer(tf.keras.layers.Layer):
    def __init__(self, sparse_feature_names, embed_dim=None, embed_l2=0.0, embed_initializer=None,
        ...
        super().__init__()
        self.sparse_feature_names = sparse_feature_names # （fea_dict1， fea_dict2，... ）
        self.embed_dim = embed_dim
        self.embed_if_output2d = embed_if_output2d # 只输出 concat 特征 （2 维）
    ...
    def build(self, input_shape):
        # sparse: (batch, sparse_fea_num)
        # seq: (batch, seq_fea_num, seq_length)
    ...
    def call(self, inputs):
        ...
        # seq: (batch, seq_fea_num, seq_length)
        sparse_tensor = inputs
        print("#SparseEmbeddingLayer 1, input shape: ", sparse_tensor.shape)
        # fixme: 这里假设了所有的 emb_dim 都一样
        # sparse: [batch_size, emb_dim] * sparse_fea_num
        # Seq: [batch_size, seq_length, emb_dim] * sparse_fea_num
        embed_tensor_list = [
            self.embed_fn_map[f'{i}'](tf.gather(params=sparse_tensor, indices=tf.constant(i, dtype=tf.int32), axis=1))
            for i in range(self.fields_width)
        ]
        # fixme: gather 这里 indices 用的是标量, 所以会降维
        print("#SparseEmbeddingLayer 2, emb_input shape: ", tf.gather(params=sparse_tensor, indices=tf.constant(0, dtype=tf.int32), axis=1).shape)
        print("#SparseEmbeddingLayer 3, embed_tensor_list size: ", len(embed_tensor_list))
        print("#SparseEmbeddingLayer 4, embed_tensor_list i shape: ", embed_tensor_list[0].shape)

        # sparse: [batch_size, emb_dim * sparse_fea_num]
        # seq: [batch_size, seq_length * sparse_fea_num, emb_dim]
        d23_tensor = tf.concat(embed_tensor_list, axis=1)
        print("#SparseEmbeddingLayer 5, d23_tensor shape: ", d23_tensor.shape)
        ...
        # sparse: [batch, sparse_fea_num, emb_dim]
        # seq: [batch, seq_fea_num, seq_length, emb_dim ]
        d34_tensor = tf.stack(embed_tensor_list, axis=1)
        print("#SparseEmbeddingLayer 6, d34_tensor shape: ", d34_tensor.shape)
        return d23_tensor, d34_tensor

# 将 dense 特征编码 embedding （不理解 ?）
# （batch_size, numerical_fea_num） -> (batch_size, numerical_fea_num, emb_dim)
