...
def make_sequence_data(
        ...
        sequence_choice_if_replace=True,
    ...
    def sequence_fn(x):
        ...
        x = numpy.random.choice(x, size=sequence_size, replace=True)
        return x
...
def make_fake_data(
        n_samples=1000,  # 数据量
        task='binary',   # 任务类型：'binary', 'regression'， ('binary', 'binary'), ['binary', 'binary']
        n_classes=2,     # 分类任务类别数
        class_weights=(0.9,), # 类别比例
        n_numerical_features=30, # 数值特征个数
        n_categorical_features=5, # 类别特征个数
        n_sequential_features=5,  # 序列特征个数
        sequence_if_stable_length=False, #
    ...
    print("#make_fake_data_debug, sparse fea vocabulary_size: ", vocabulary_size)
    # 不包含 0
    ...
    if n_classes <= 2:
        ...
        # >= n_classes + 3 if n_classes > 2 else `any`.
        n_features = max([n_numerical_features, n_classes + 3])

    numerical_features, y = make_classification(
        n_classes=n_classes,    # 类别个数
        weights=class_weights,  # 类别比列
        n_samples=n_samples,    # 样本量
        n_features=n_features,  # 特征个数
        n_informative=n_classes, # 信息性特征
        n_redundant=2,  # 冗余特征，informative 特征随机线性组合
        n_repeated=0,   # 重复特征，其他特征的随机选择
        shuffle=shuffle,
        random_state=seed,
    )
    ...
    print("#make_fake_data_debug, categorical_features shape: ", categorical_features.shape)
